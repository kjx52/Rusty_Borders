...
/*
     __  __  __   _                    ______    ______   __   __
     |00 |00 |00 [=0                   |000000  | 000000  |00  |00
     |00 |00 |00 _000 ______  _____   |0000000  |0000000  |00__|00
     |00 |00 |00 | |  |000000 |00000  |000____  |000____  |0000000
     |00 |00 |00 |000 |000000  00000  |00000000 |00000000 |0000000
     |00_|00_|00 |000 |00 |00          ____|000  ____|000 |00  |00
      0000000000 |000 |00 |00         |00000000 |00000000 |00  |00
       00000000                        00000000  00000000  00   00
...
main()
{
	...
	printf("\n\033[1;34m/*Rusty_Boders 危墙 全面部署程序*/\n");
	printf(" 作者：Jessarin000\n");
	printf(" 日期：2024-05-25\n");
	printf(" 版本：1.0\033[0m\n\n");
	printf("    本项目为原 SSH 蜜罐项目 Windows 版本的 Rust 衍生版，用于在 Windows 环境下自动\n");
	printf("化部署 SSH 弱口令蜜罐项目。本项目原理为修改原SSH 默认连接终端 CMD 为指定伪终端\n");
	printf("Rusty_Boders，从而模拟Linux下的 Chroot 环境，从而限制攻击者的路径变迁以及代码执行。\n\n");
	printf("    本项目：\n");
	printf("        默认创建账户为 kali，密码为 kjx00000；\n");
	printf("        默认创建目录为 C:\\Users\\kali\\Shell\n");
	printf("        默认蜜罐终端为 RyB。\n");
	printf("        （\033[1;31m注意：本项目目前尚不兼容SSH-Honeypot For Windows中的WinBacsh终端，下一版本作者将根据反馈决定是否合并两项目\033[0m）\n");
	printf("        默认情况下，攻击者在蜜罐中可使用的命令为：cd，ls，cat，cls，pwd，whoami，hostname，id，touch，mkdir，rm，wipe，su，help。\n\n");
	...
	if((access("C:\\Users\\Public\\SSHShell\\Rusty_Borders_危墙.exe",F_OK))!=-1)
	{
		if(system("powershell -c \"New-ItemProperty -Path \'HKLM:\\SOFTWARE\\OpenSSH\' -Name DefaultShell -Value \'C:\\Users\\Public\\SSHShell\\Rusty_Borders_危墙.exe\' -PropertyType String -Force 1>$null 2>&1\"")!=-1)
		{
			printf("完成。\033[0m\n");
		}
		else
	...
	if(system("Icacls \"C:\\Users\\Public\\SSHShell\\Rusty_Borders_危墙.exe\" /grant  kali:RX 2>$null 1>&2")==-1)
	{
		i=1;
	...
	printf("[\033[33m!\033[0m] 在某些环境下，有可能出现失败但未报错的情况（如Microsoft Defender 自动阻止 CMD 添加用户。）\n");
	printf("    所以出于保护地球的考量，请确认所有工作成功完成后再启动蜜罐。\n\n");
	
	printf("[*] 如若第一遍运行ByB时使用的是Administrator账户，则（很有）可能在以kali身份启动时遇到权限不足问题。\n");
	printf("    请删除C:\\Users\\Public\\History 文件夹下的命令记录文件。\n");
	
	printf("\033[1m[\033[1;32m+\033[0m\033[1m]\033[0m \033[1m    如果没有任何警告或错误，那么现在Windows SSH 蜜罐\033[0m\033[30;47m应该\033[0m\033[1m已经在您的计算机下部署完成了。\n");
